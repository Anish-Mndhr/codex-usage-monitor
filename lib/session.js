...
async function summarizeSessionFile(transcriptPath, options = {}) {
  ...
  const selectedRecords = state.exactUsageRecords ? state.exactRecords : [...state.legacyRecords.values()];
  ...
  for (const record of selectedRecords) {
    if (record.costUsd == null) record.costUsd = costForUsage(record.model, record.usage, pricingConfig);
    record.costStatus = record.costUsd == null ? 'unknown' : 'estimated';
  }
  Object.defineProperty(summary, 'usageRecords', {
    value: selectedRecords.sort(compareUsageRecords),
    enumerable: false,
  });
...
function consumeRecord(record, summary, state) {
  ...
  state.legacyRecords.set(turnId, usageRecord(`legacy:${turnId}`, turnId, model, timestamp, summary.latestUsage));
...
function consumeExactUsage(payload, timestamp, summary, state) {
  ...
  const recordId = `exact:${state.exactRecords.length + 1}`;
  const record = usageRecord(recordId, turnId, model, timestamp, usage);
  record.costUsd = details?.usd ?? null;
  record.costStatus = details ? 'estimated' : 'unknown';
  state.exactRecords.push(record);
...
function usageRecord(recordId, turnId, model, timestamp, usage) {
  return {
    recordId,
    turnId,
    model,
...
function compareUsageRecords(a, b) { return String(a.occurredAt || '').localeCompare(String(b.occurredAt || '')); }
