...
function migrate(db) {
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS usage_records_time_idx ON usage_records(occurred_at);
    CREATE INDEX IF NOT EXISTS usage_records_session_idx ON usage_records(session_id);
    PRAGMA user_version = 2;
  `);
  const columns = db.prepare('PRAGMA table_info(ingestion_files)').all();
  if (!columns.some((column) => column.name === 'ingestion_version')) {
    db.exec('ALTER TABLE ingestion_files ADD COLUMN ingestion_version INTEGER NOT NULL DEFAULT 0;');
  }
}
...
async function syncSessions(options = {}) {
  ...
  try {
    for (const file of files) {
      ...
      const previous = db.prepare('SELECT file_size, file_mtime_ms, ingestion_version FROM ingestion_files WHERE transcript_path = ?').get(file);
      if (!options.all && previous && previous.file_size === stat.size
        && previous.file_mtime_ms === Math.floor(stat.mtimeMs)
        && previous.ingestion_version === INGESTION_VERSION) {
        result.skipped++;
        continue;
      }
...
function saveSummary(db, summary, options = {}) {
  ...
  try {
    ...
    db.prepare('DELETE FROM usage_records WHERE session_id = ?').run(summary.sessionId);
    const insertUsageRecord = db.prepare(`INSERT INTO usage_records VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
    for (const record of summary.usageRecords || []) {
      const item = record.usage || zeroUsage();
      insertUsageRecord.run(summary.sessionId, record.recordId, iso(record.occurredAt), record.turnId,
        record.model, item.inputTokens, item.cachedInputTokens, item.cacheWriteInputTokens,
        item.outputTokens, item.reasoningOutputTokens, item.totalTokens, record.costUsd,
        record.costStatus || (record.costUsd == null ? 'unknown' : 'estimated'));
    }

    db.prepare(`INSERT INTO ingestion_files
      (transcript_path, session_id, file_size, file_mtime_ms, ingested_at, ingestion_version)
      VALUES (?, ?, ?, ?, ?, ?)
      ...
      ingested_at=excluded.ingested_at, ingestion_version=excluded.ingestion_version`)
      .run(summary.transcriptPath, summary.sessionId, summary.fileSize, summary.fileMtimeMs, now, INGESTION_VERSION);
    db.exec('COMMIT');
    return true;
  } catch (error) {
    db.exec('ROLLBACK');
  ...
  try {
    ...
    if (transcriptPath) {
      ...
      if (summary) {
        ...
        attachTodayUsage(db, summary, options.now || new Date());
        return summary;
      }
    }
...
function queryTodayUsage(db, options = {}) {
  ...
  const row = db.prepare(`SELECT COUNT(DISTINCT session_id) AS sessions,
    COUNT(DISTINCT session_id || char(31) || COALESCE(turn_id, record_id)) AS turns,
    ...
    COALESCE(SUM(CASE WHEN cost_status != 'estimated' THEN 1 ELSE 0 END),0) AS incomplete
    FROM usage_records WHERE ${where.join(' AND ')}`).get(...params);
  const usage = {
  ...
  return {
    ...
    turnCount: row.turns,
    usage,
    cost: { usd: row.cost_usd, complete: row.incomplete === 0 },
  };
}

function attachTodayUsage(db, summary, now = new Date()) {
  summary.sessionTodayUsage = queryTodayUsage(db, { now, sessionId: summary.sessionId });
  summary.allTodayUsage = queryTodayUsage(db, { now });
  return summary;
}
...
function getSession(db, sessionId) {
  ...
  if (!session) {
    const matches = db.prepare('SELECT * FROM sessions WHERE substr(session_id, 1, ?) = ? LIMIT 2')
      .all(String(sessionId).length, sessionId);
  ...
  return {
    ...session,
    turns: db.prepare('SELECT * FROM turns WHERE session_id = ? ORDER BY updated_at').all(session.session_id),
    models: db.prepare('SELECT * FROM session_models WHERE session_id = ? ORDER BY input_tokens DESC').all(session.session_id),
  };
}
